const Map<String, String> arQA = {
  "Popular Locations": "المناطق الشائعة",
  "Featured Properties": "العقارات المميزة",
  "Property Categories": "فئات العقارات",

  "Villa": "فيلا",
  "Apartment": "شقة",
  "Townhouse": "تاون هاوس",
  "Office": "مكتب",
  "Commercial": "تجاري",
  "Land": "أرض",

  "Home": "الرئيسية",
  "Chats": "المحادثات",
  "My Listings": "عقاراتي",
  "Profile": "الملف الشخصي",
  "More": "المزيد",

  "Settings": "الإعدادات",
  "Language": "اللغة",
  "English": "الإنجليزية",
  "Arabic": "العربية",

  "Search": "بحث",
  "View All": "عرض الكل",
  "Continue": "متابعة",
  "Cancel": "إلغاء",
  "Save": "حفظ",
  "Submit": "إرسال",

  "Why choose Villas Qatar?": "لماذا تختار فلل قطر؟",
  "Wide range of verified properties": "مجموعة واسعة من العقارات الموثقة",

  "Learn More": "اعرف المزيد",
  "Doha, Qatar": "الدوحة، قطر",
  "Search property": "ابحث عن عقار",

  "Start as Guest": "ابدأ كضيف",
  "Explore the app without an account": "استكشف التطبيق بدون حساب",

  "Continue with WhatsApp": "المتابعة عبر واتساب",
  "Login with WhatsApp": "تسجيل الدخول عبر واتساب",
  "Login with ": "تسجيل الدخول عبر ",
  "WhatsApp": "واتساب",
  "Enter your WhatsApp number to\nreceive an OTP":
      "أدخل رقم واتساب الخاص بك\nلتلقي رمز التحقق",
  "We will send an OTP to your WhatsApp to verify your account":
      "سنرسل رمز التحقق (OTP) إلى واتساب الخاص بك للتحقق من حسابك",
  "WhatsApp Number": "رقم واتساب",
  "Verify ": "تحقق",
  "OTP": "رمز التحقق",
  "Enter the 6-digit code sent to\n@phone":
      "أدخل الرمز المكون من 6 أرقام المرسل إلى\n@phone",
  "Resend OTP in 00:25": "إعادة إرسال رمز التحقق خلال 00:25",

  "Full Name": "الاسم الكامل",
  "Email Address": "البريد الإلكتروني",

  "Edit Profile": "تعديل الملف الشخصي",
  "Logout": "تسجيل الخروج",
  "See all": "عرض الكل",
  "Villas": "فلل",
  "Apartments": "شقق",
  "Offices": "مكاتب",
  "Townhouses": "تاون هاوس",
  "Find your perfect\nproperty in Qatar": "اعثر على\nالعقار المثالي في قطر",

  "Search property...": "ابحث عن عقار",
  "Find Your": "اعثر على",

  "Dream Property": "العقار المثالي",

  "Buy": "شراء",
  "Rent": "إيجار",
  "PG/Co-living": "سكن مشترك",

  "Search by location or property": "ابحث حسب الموقع أو العقار",
  "Property Type": "نوع العقار",

  "Search Properties": "ابحث عن العقارات",

  "Price Range": "نطاق السعر",
  "Filters": "الفلاتر",

  "Popular Localities": "المناطق الشائعة",
  "See All": "عرض الكل",

  "FEATURED": "مميز",
  "For Sale": "للبيع",

  "Featured Agents": "الوكلاء المميزون",
  "Explore Qatar": "استكشف قطر",
  "Dream Villa": "فيلا الأحلام",
  "in Qatar": "في قطر",
  "Discover premium villas and properties in the best locations across Qatar":
      "اكتشف الفلل والعقارات الفاخرة في أفضل المواقع في قطر",
  "Search location, property name or type":
      "ابحث حسب الموقع أو اسم العقار أو نوعه",
  "Properties": "العقارات",
  "New": "جديد",
  "Launches": "إطلاقات",
  "Luxury": "فاخر",
  "Collection": "مجموعة",
  "Shortlist": "المفضلة",

  "Popular Areas": "المناطق الشائعة",
  "Explore top locations": "استكشف أفضل المناطق",
  "Buying Guide": "دليل الشراء",
  "Step by step process": "خطوة بخطوة",
  "Investment Guide": "دليل الاستثمار",
  "Maximize your returns": "حقق أقصى عائد من استثمارك",
  "Market Trends": "اتجاهات السوق",
  "Stay updated": "ابقَ على اطلاع",
  // Arabic
  "Find your": "اعثر على",
  "dream villa": "فيلا أحلامك",

  // ================= SEARCH / PROPERTY LISTING =================
  "Search property name": "ابحث باسم العقار",
  "Near You": "بالقرب منك",
  "Wishlist": "قائمة المفضلة",
  "Saved": "المحفوظات",
  "Featured Dealers": "الوكلاء المميزون",

  "Buy, rent and discover the best properties around you":
      "اشترِ أو استأجر واكتشف أفضل العقارات من حولك",

  "No Properties Found": "لم يتم العثور على عقارات",
  "No more properties": "لا توجد عقارات أخرى",
  "Try changing your search or filters": "حاول تغيير البحث أو الفلاتر",

  "Filter Properties": "تصفية العقارات",
  "Refine your search results": "حسّن نتائج البحث",

  "Min Price": "الحد الأدنى للسعر",
  "Max Price": "الحد الأقصى للسعر",

  // ================= PROPERTY DETAILS =================
  "Property Details": "تفاصيل العقار",
  "Beds": "غرف النوم",
  "Baths": "الحمامات",
  "Area": "المساحة",
  "Furnishing": "التأثيث",

  "Furnished": "مفروش",
  "Semi-Furnished": "شبه مفروش",
  "Unfurnished": "غير مفروش",

  "Reset": "إعادة تعيين",
  "Apply Filters": "تطبيق الفلاتر",

  // ================= MY PROPERTIES =================
  "My Properties": "عقاراتي",
  "Search by property name or location": "ابحث باسم العقار أو الموقع",

  "Try a different search or filter, or list a new property":
      "جرّب بحثًا أو فلترًا مختلفًا، أو أضف عقارًا جديدًا",

  "ACTIVE": "نشط",
  "PENDING": "قيد الانتظار",
  "INACTIVE": "غير نشط",

  "For Rent": "للإيجار",

  "List Property": "أضف عقارًا",
  "All": "الكل",

  // ================= PROPERTY STEPS =================
  "Basic Info": "المعلومات الأساسية",
  "Details": "التفاصيل",
  "Features": "المميزات",
  "Location": "الموقع",
  "Media": "الوسائط",

  "List Your Property": "أدرج عقارك",

  // ================= OWNER DETAILS =================
  "Property Owner Details": "تفاصيل مالك العقار",
  "Enter your details to get started": "أدخل بياناتك للبدء",

  "Enter your full name": "أدخل اسمك الكامل",

  "Contact Number": "رقم التواصل",
  "Enter mobile number": "أدخل رقم الهاتف",

  "Send OTP": "إرسال رمز التحقق",

  "Verification Code": "رمز التحقق",
  "Enter 6-digit OTP": "أدخل رمز التحقق المكون من 6 أرقام",
  "Resend OTP": "إعادة إرسال رمز التحقق",

  "Enter your email address": "أدخل بريدك الإلكتروني",

  "Short Description": "وصف مختصر",
  "Write a short description about your property":
      "اكتب وصفًا مختصرًا عن عقارك",

  "WhatsApp Verification": "التحقق عبر واتساب",

  "Your contact number is verified": "تم التحقق من رقم التواصل الخاص بك",

  "Your contact number is not verified":
      "لم يتم التحقق من رقم التواصل الخاص بك",

  // ================= PROPERTY INFORMATION =================
  "Tell us about your property": "أخبرنا عن عقارك",

  "Property Name": "اسم العقار",
  "Enter property name": "أدخل اسم العقار",
  "Select property type": "اختر نوع العقار",

  "Bedrooms": "غرف النوم",
  "e.g. 4": "مثال: 4",

  "Bathrooms": "الحمامات",
  "e.g. 3": "مثال: 3",

  "Living Rooms": "غرف المعيشة",
  "e.g. 2": "مثال: 2",

  "Parking Spaces": "مواقف السيارات",

  "Property For": "العقار مخصص لـ",

  "Sale": "بيع",

  "Price": "السعر",
  "Enter price": "أدخل السعر",

  "Area (sqm)": "المساحة (م²)",
  "Enter property area": "أدخل مساحة العقار",

  "Year Built": "سنة البناء",
  "e.g. 2024": "مثال: 2024",

  "Floor Number": "رقم الطابق",
  "e.g. 5": "مثال: 5",

  "Total Floors": "إجمالي الطوابق",
  "e.g. 20": "مثال: 20",

  // ================= FEATURES & AMENITIES =================
  "Features & Amenities": "المميزات ووسائل الراحة",

  "Select the amenities and nearby facilities":
      "اختر وسائل الراحة والمرافق القريبة",

  "Other Features": "ميزات أخرى",

  "Add any additional features not listed above":
      "أضف أي ميزات إضافية غير مدرجة أعلاه",

  "Other (Please specify)": "أخرى (يرجى التحديد)",
  "Enter other features": "أدخل الميزات الأخرى",

  "Amenities": "وسائل الراحة",
  "Nearby Tags": "المرافق القريبة",

  "Add": "إضافة",

  "No amenities selected": "لم يتم اختيار أي وسائل راحة",

  "Select Amenities": "اختر وسائل الراحة",

  "Choose the amenities available in your property.":
      "اختر وسائل الراحة المتوفرة في عقارك.",

  "Done": "تم",

  "No nearby tags selected": "لم يتم اختيار أي مرافق قريبة",

  "No furnishing option selected": "لم يتم اختيار نوع التأثيث",

  // ================= LOCATION =================
  "Property Location": "موقع العقار",

  "Tell buyers where your property is located": "أخبر المشترين بموقع عقارك",

  "Address Line 1": "العنوان - السطر الأول",
  "Enter address": "أدخل العنوان",

  "Address Line 2": "العنوان - السطر الثاني",
  "Apartment / Building / Street": "الشقة / المبنى / الشارع",

  "Name of Area": "اسم المنطقة",

  "Municipality / City": "البلدية / المدينة",
  "Enter municipality": "أدخل اسم البلدية",

  "Landmark": "معلم بارز",
  "Nearby landmark": "معلم قريب",

  "Latitude": "خط العرض",
  "Longitude": "خط الطول",

  "Google Map integration can be added here for selecting the exact location.":
      "يمكن إضافة خرائط Google هنا لتحديد الموقع الدقيق.",

  // ================= PHOTOS & VIDEOS =================
  "Photos & Videos": "الصور ومقاطع الفيديو",

  "Add high quality photos and videos to attract more buyers":
      "أضف صورًا ومقاطع فيديو عالية الجودة لجذب المزيد من المشترين",

  "Property Photos": "صور العقار",

  "Upload clear and attractive photos (Max 20 photos)":
      "حمّل صورًا واضحة وجذابة (بحد أقصى 20 صورة)",

  "Add Photos": "إضافة صور",

  "JPG, PNG up to 10MB each": "JPG وPNG بحجم يصل إلى 10 ميجابايت لكل صورة",

  "Property Videos": "فيديوهات العقار",
  "(Optional)": "(اختياري)",

  "Upload video tour of your property (Max 3 videos)":
      "حمّل جولة فيديو للعقار (بحد أقصى 3 فيديوهات)",

  "Add Video": "إضافة فيديو",
  "Video Selected": "تم اختيار الفيديو",

  "MP4, MOV up to 50MB each": "MP4 وMOV بحجم يصل إلى 50 ميجابايت لكل فيديو",

  "Video Guidelines": "إرشادات الفيديو",

  "Tip: Properties with photos get 10x more interest!":
      "نصيحة: العقارات التي تحتوي على صور تحظى باهتمام أكبر بـ10 مرات!",

  "Make sure to add high quality photos and videos":
      "تأكد من إضافة صور ومقاطع فيديو عالية الجودة",

  // ================= REVIEW / SUBMIT =================
  "Review & Publish": "مراجعة ونشر",
  "Save & Continue": "حفظ ومتابعة",
  // Chat Screen
  "Can we schedule a visit?": "هل يمكننا تحديد موعد للزيارة؟",
  "Is the price negotiable?": "هل السعر قابل للتفاوض؟",
  "What is the payment plan?": "ما هي خطة الدفع؟",
  "Seller": "البائع",
  "Typing...": "يكتب...",
  "Online": "متصل",
  "No Messages Yet": "لا توجد رسائل بعد",
  "Type a message...": "اكتب رسالة...",
  "Gallery": "المعرض",
  "Camera": "الكاميرا",
  // Chat List Screen
  "Search by property or seller": "ابحث حسب العقار أو البائع",
  "No Conversations": "لا توجد محادثات",
  "My Chats": "محادثاتي",
  "My Visits": "زياراتي",
  "Property": "العقار",

  "No messages yet": "لا توجد رسائل بعد",
  "Chat with ": "الدردشة مع ",
  "Yesterday": "أمس",
  // Chat Start Screen
  "Image unavailable": "الصورة غير متاحة",
  "Property Consultant": "مستشار عقاري",

  // Report User
  "Report User": "الإبلاغ عن المستخدم",
  "Unable to identify the user": "تعذر تحديد المستخدم",
  "Please select a reason": "يرجى اختيار سبب",
  "Report submitted successfully": "تم إرسال البلاغ بنجاح",
  "Unable to submit report": "تعذر إرسال البلاغ",

  // Report Reasons
  "Fake profile": "حساب مزيف",
  "Fraud or scam": "احتيال أو خداع",
  "Inappropriate behavior": "سلوك غير لائق",
  "Spam or unwanted messages": "رسائل مزعجة أو غير مرغوب فيها",
  "Misleading information": "معلومات مضللة",
  "Other": "أخرى",

  // Report Form
  "Tell us why you're reporting this user.":
      "أخبرنا لماذا تقوم بالإبلاغ عن هذا المستخدم.",
  "Reason for Report": "سبب الإبلاغ",
  "Additional Details": "تفاصيل إضافية",
  "Describe the issue": "صف المشكلة",
  "Your report will be reviewed by our support team.":
      "سيتم مراجعة بلاغك من قبل فريق الدعم لدينا.",
  "Submit Report": "إرسال البلاغ",
  "Start Conversation": "بدء المحادثة",
  "Ask anything about this property.\nThe property consultant will reply as soon as possible.":
      "اسأل عن أي شيء يتعلق بهذا العقار.\nسيرد عليك المستشار العقاري في أقرب وقت ممكن.",
  "You can ask about": "يمكنك الاستفسار عن",
  "Price Negotiation": "التفاوض على السعر",
  "Payment Plans": "خطط الدفع",
  "Property Availability": "توفر العقار",
  "I can pay up to ": "يمكنني الدفع حتى ",
  "Is this property still available?": "هل هذا العقار لا يزال متاحًا؟",
  "Pending": "قيد الانتظار",
  "Accepted": "مقبول",
  "Confirmed": "مؤكد",
  "Rejected": "مرفوض",
  "Cancelled": "ملغى",
  "Completed": "مكتمل",
  "My Wishlist": "قائمة المفضلة",
  "Saved Properties": "العقارات المحفوظة",
  "properties saved": "عقارات محفوظة",
  "Search saved properties": "ابحث في العقارات المحفوظة",
  "Your wishlist is empty": "قائمة المفضلة فارغة",
  "Save properties you love and find them easily here.":
      "احفظ العقارات التي تعجبك لتتمكن من العثور عليها بسهولة هنا.",
  "No saved properties found": "لم يتم العثور على عقارات محفوظة",
  "Visit Requests": "طلبات الزيارة",
  "My Property": "عقاري",

  // ================= PROPERTY DETAILS SCREEN =================
  "Property Owner": "مالك العقار",
  "Call": "اتصال",
  "Reviews": "التقييمات",

  "Overview": "نظرة عامة",

  "Built-up Area": "المساحة المبنية",
  "Floor": "الطابق",
  "Floors": "الطوابق",
  "Parking": "مواقف السيارات",
  "Listed On": "تاريخ الإدراج",

  "View on Maps": "عرض على الخريطة",

  "Verified": "موثّق",
  "Not Verified": "غير موثّق",
  "away": "بعيد",
  "sqm": "م²",

  // ================= NEARBY TAGS =================
  "School": "مدرسة",
  "Metro": "مترو",
  "Hospital": "مستشفى",
  "Clinic": "عيادة",
  "Mall": "مركز تسوق",
  "Airport": "مطار",
  "Beach": "شاطئ",
  "Pharmacy": "صيدلية",
  "Mosque": "مسجد",
  "Church": "كنيسة",
  "Boost Property": "تعزيز العقار",

  "Purpose": "الغرض",

  "Mark as Sold": "تحديد كمباع",
  "Make an Offer": "تقديم عرض",
  "Property Price": "سعر العقار",
  "Review the listed price before making your offer":
      "راجع السعر المُدرج قبل تقديم عرضك",
  "Your Offer": "عرضك",
  "Enter the amount you would like to offer": "أدخل المبلغ الذي ترغب في تقديمه",
  "Listed Price": "السعر المُدرج",
  "Schedule": "جدولة",
  "Send Offer": "إرسال العرض",
  "Chat Seller": "محادثة البائع",
  "Offer Amount": "مبلغ العرض",

  "Schedule a Visit": "جدولة زيارة",
  "Select Visit Date": "اختر تاريخ الزيارة",
  "Available Time": "الأوقات المتاحة",
  "Add a note (Optional)": "أضف ملاحظة (اختياري)",
  "Book Visit": "حجز زيارة",

  "Choose Boost Plan": "اختر خطة التعزيز",
  "Select how you want to promote your listing":
      "اختر الطريقة التي تريد بها الترويج لإعلانك",

  "Search boost plans": "ابحث عن خطط التعزيز",

  "Loading boost plans...": "جارٍ تحميل خطط التعزيز...",

  "No plans found": "لم يتم العثور على خطط",
  "Try searching with another keyword": "حاول البحث باستخدام كلمة أخرى",

  "POPULAR": "الأكثر شيوعًا",

  "Secure payment powered by Stripe": "دفع آمن عبر Stripe",

  "Total": "الإجمالي",

  "Unable to load boost plans": "تعذر تحميل خطط التعزيز",

  "Try Again": "حاول مرة أخرى",

  "No boost plans available": "لا توجد خطط تعزيز متاحة",

  "Please check again later.": "يرجى التحقق مرة أخرى لاحقًا.",

  "Unable to Boost": "تعذر تعزيز العقار",
  "Property ID is missing.": "معرّف العقار غير موجود.",

  "Ready for Payment": "جاهز للدفع",
  "Requested by": "مقدم الطلب",
  // ================= SETTINGS SCREEN =================
  "Account": "الحساب",
  "My Featured Properties": "عقاراتي المميزة",

  // Preferences
  "Preferences": "التفضيلات",

  "Notifications": "الإشعارات",

  // Support
  "Support": "الدعم",
  "Help Center": "مركز المساعدة",
  "Contact Us": "اتصل بنا",
  "About": "حول التطبيق",

  // Profile
  "View Profile": "عرض الملف الشخصي",

  "Are you sure you want to logout from your account?":
      "هل أنت متأكد أنك تريد تسجيل الخروج من حسابك؟",

  "My Tickets": "طلباتي",

  "How can we help?": "كيف يمكننا مساعدتك؟",
  "Send us your issue and our support team will assist you.":
      "أرسل لنا مشكلتك وسيساعدك فريق الدعم لدينا.",

  "What can we help you with?": "كيف يمكننا مساعدتك؟",

  // ================= SUPPORT CATEGORIES =================
  "General Support": "الدعم العام",
  "Questions, account help or any general issue.":
      "الاستفسارات أو المساعدة في الحساب أو أي مشكلة عامة.",

  "Payment Issue": "مشكلة في الدفع",
  "Payment, billing, transaction or subscription issues.":
      "مشكلات الدفع أو الفوترة أو المعاملات أو الاشتراكات.",

  // ================= ISSUE FORM =================
  "Tell us about the issue": "أخبرنا عن المشكلة",

  "Subject": "الموضوع",
  "e.g. Payment failed": "مثال: فشل الدفع",
  "Briefly describe your issue": "صف مشكلتك باختصار",

  "Description": "الوصف",

  "Explain the payment issue, transaction details, or what went wrong...":
      "اشرح مشكلة الدفع أو تفاصيل المعاملة أو ما الذي حدث...",

  "Tell us what happened and how we can help...":
      "أخبرنا بما حدث وكيف يمكننا مساعدتك...",

  "Please provide enough details so our support team can understand and resolve your issue.":
      "يرجى تقديم تفاصيل كافية حتى يتمكن فريق الدعم لدينا من فهم مشكلتك وحلها.",

  "Submit Request": "إرسال الطلب",

  // ================= VALIDATION / MESSAGES =================
  "Please enter a subject for your request.": "يرجى إدخال موضوع لطلبك.",

  "Please describe the issue.": "يرجى وصف المشكلة.",

  "Unable to identify the logged-in user.": "تعذر تحديد المستخدم المسجل دخوله.",

  "Your support request has been submitted successfully.":
      "تم إرسال طلب الدعم الخاص بك بنجاح.",

  "Unable to submit. Please try again.":
      "تعذر الإرسال. يرجى المحاولة مرة أخرى.",

  // ================= SUPPORT TICKET DETAILS =================
  "Ticket Details": "تفاصيل طلب الدعم",

  "Ticket Information": "معلومات الطلب",
  "Submitted By": "مقدم الطلب",
  "Assigned To": "تم التعيين إلى",
  "Your Request": "طلبك",
  "Support Replies": "ردود الدعم",

  "Reply": "رد",
  "Replies": "ردود",

  // ================= TICKET INFORMATION =================
  "Ticket ID": "معرّف الطلب",
  "Category": "الفئة",
  "Status": "الحالة",
  "Listing ID": "معرّف الإعلان",
  "Reported User ID": "معرّف المستخدم المُبلّغ عنه",
  "Reference ID": "الرقم المرجعي",
  "Created": "تاريخ الإنشاء",
  "Last Updated": "آخر تحديث",

  "Reported User": "مستخدم مُبلّغ عنه",
  "Reported Listing": "إعلان مُبلّغ عنه",

  // ================= STATUS =================
  "OPEN": "مفتوح",
  "IN PROGRESS": "قيد المعالجة",
  "RESOLVED": "تم الحل",
  "CLOSED": "مغلق",

  // ================= REPLIES / USERS =================
  "Support Team": "فريق الدعم",

  "Support Agent": "موظف الدعم",
  "User": "المستخدم",

  "No replies yet": "لا توجد ردود بعد",
  "Our support team will respond here.": "سيرد فريق الدعم لدينا هنا.",

  "No description provided.": "لم يتم تقديم وصف.",

  // ================= USER INFORMATION =================
  "User ID": "معرّف المستخدم",
  "Email": "البريد الإلكتروني",

  "Submitter information is not available.": "معلومات مقدم الطلب غير متاحة.",

  "Not available": "غير متاح",

  // ================= ACTIONS =================
  "Retry": "إعادة المحاولة",

  // ================= MY TICKETS ================// Ticket statuses
  "Open": "مفتوح",
  "In Progress": "قيد المعالجة",
  "Resolved": "تم الحل",
  "Closed": "مغلق",

  // Empty state
  "No Support Tickets": "لا توجد طلبات دعم",
  "Your submitted support requests will appear here.":
      "ستظهر طلبات الدعم التي قدمتها هنا.",
  "Search featured properties": "ابحث في العقارات المميزة",
  "Manage your currently featured properties": "إدارة عقاراتك المميزة حاليًا",

  // ================= AI PRICE ESTIMATOR =================
  "AI Price Estimator": "مُقدّر أسعار العقارات بالذكاء الاصطناعي",

  "Estimate Smarter": "قدّر بذكاء",
  "Price Better.": "وسَعّر بشكل أفضل.",
  "Get an AI-powered estimate of your\n property value in seconds.":
      "احصل على تقدير لقيمة عقارك بالذكاء الاصطناعي خلال ثوانٍ.",

  "Enter city, locality or area": "أدخل المدينة أو المنطقة أو الحي",
  "Area (sqft)": "المساحة (قدم²)",
  "e.g. 1200": "مثال: 1200",
  "sqft": "قدم²",

  "Villa / House": "فيلا / منزل",
  "Plot": "أرض",

  "Select amenities": "اختر المرافق",

  "You can select multiple amenities": "يمكنك اختيار عدة مرافق",
  "Amenities are not available.": "المرافق غير متاحة.",
  "Clear All": "مسح الكل",
  "selected": "محدد",
  "Apply": "تطبيق",

  // Highlights
  "Share some highlights (Optional)": "أضف بعض المميزات (اختياري)",
  "e.g. Sea view, Near metro, Gated community...":
      "مثال: إطلالة بحرية، بالقرب من المترو، مجمع سكني مغلق...",

  // Estimate Card
  "Estimated Price": "السعر التقديري",
  "Get an estimate": "احصل على تقدير",
  "Enter property details below": "أدخل تفاصيل العقار أدناه",

  // Property Details
  "BHK": "غرف النوم",
  "Select": "اختر",
  "Select BHK": "اختر عدد غرف النوم",

  "Select Bathrooms": "اختر عدد الحمامات",
  "Bathroom": "حمام",

  "Select Furnishing": "اختر حالة التأثيث",
  "Furnishing options are not available.": "خيارات التأثيث غير متاحة.",

  "Select Floor": "اختر الطابق",
  "Ground Floor": "الطابق الأرضي",

  "Select Total Floors": "اختر إجمالي عدد الطوابق",

  "Age of Property": "عمر العقار",
  "Select Property Age": "اختر عمر العقار",
  "New / Under 1 Year": "جديد / أقل من سنة",
  "1 - 5 Years": "1 - 5 سنوات",
  "5 - 10 Years": "5 - 10 سنوات",
  "10+ Years": "أكثر من 10 سنوات",

  "Any": "أي",

  "Select Parking Availability": "اختر توفر موقف السيارات",
  "Available": "متوفر",
  "Not Available": "غير متوفر",

  // CTA
  "Get AI Estimate": "احصل على تقدير بالذكاء الاصطناعي",
  "View estimated price & range": "عرض السعر التقديري والنطاق",

  // Validation
  "Please enter the property location.": "يرجى إدخال موقع العقار.",
  "Please enter a valid property area.": "يرجى إدخال مساحة عقار صحيحة.",
  "Please select BHK.": "يرجى اختيار عدد غرف النوم.",
  "Please select bathrooms.": "يرجى اختيار عدد الحمامات.",
  "Please select furnishing status.": "يرجى اختيار حالة التأثيث.",
  "Unable to estimate price.": "تعذر تقدير السعر.",

  // Result
  "AI Estimated Value": "القيمة المقدرة بالذكاء الاصطناعي",
  "Minimum": "الحد الأدنى",
  "Maximum": "الحد الأقصى",

  // Footer
  "Your information is secure and private": "معلوماتك آمنة وخاصة",

  // --- i18n sweep additions ---
  "Success": "تم بنجاح",
  "Visit scheduled successfully": "تم جدولة الزيارة بنجاح",
  "Error": "خطأ",
  "Visit ID is missing": "معرّف الزيارة مفقود",
  "Invalid price estimation response": "استجابة غير صالحة لتقدير السعر",
  "Property ID is missing": "معرّف العقار مفقود",
  "Invalid featured properties response": "استجابة غير صالحة للعقارات المميزة",
  "Unable to load featured properties": "تعذّر تحميل العقارات المميزة",
  "Property details are not available": "تفاصيل العقار غير متوفرة",
  "Plan": "الخطة",
  "Select Property": "اختر العقار",
  "Choose the property you want to promote": "اختر العقار الذي تريد الترويج له",
  "Reach more buyers": "الوصول إلى المزيد من المشترين",
  "Boost your property to increase visibility and get more enquiries.":
      "عزّز عقارك لزيادة ظهوره والحصول على المزيد من الاستفسارات.",
  "Loading your properties...": "جارٍ تحميل عقاراتك...",
  "Couldn't load properties": "تعذّر تحميل العقارات",
  "Please check your connection and try again.":
      "يرجى التحقق من اتصالك والمحاولة مرة أخرى.",
  "Add a property before creating a boost.":
      "أضف عقارًا قبل إنشاء عملية ترويج.",
  "Load more properties": "تحميل المزيد من العقارات",
  "Price not available": "السعر غير متوفر",
  "Please select a property to boost": "يرجى اختيار عقار للترويج له",
  "Review": "المراجعة",
  "Draft saved successfully": "تم حفظ المسودة بنجاح",
  "Some images could not be uploaded.": "تعذّر رفع بعض الصور.",
  "Property listed successfully.": "تم إدراج العقار بنجاح.",
  "Image upload failed": "فشل رفع الصورة",
  "Failed to upload cover image": "فشل رفع صورة الغلاف",
  "Failed to upload image": "فشل رفع الصورة",
  "OTP sent successfully": "تم إرسال رمز التحقق بنجاح",
  "Phone verified successfully": "تم التحقق من رقم الهاتف بنجاح",
  " / month": " / شهريًا",
  "Your contact number has been verified.":
      "تم التحقق من رقم التواصل الخاص بك.",
  "Verify your WhatsApp number before publishing this property.":
      "يرجى التحقق من رقم الواتساب قبل نشر هذا العقار.",
  "Search location": "ابحث عن الموقع",
  "Upload Cover Image": "رفع صورة الغلاف",
  "JPG, PNG up to 10MB": "JPG أو PNG حتى 10 ميجابايت",
  "Cover": "الغلاف",
  "Phone": "الهاتف",
  "WhatsApp Verified": "تم التحقق من الواتساب",
  "Yes": "نعم",
  "No": "لا",
  "Municipality": "البلدية",
  "Photos & Media": "الصور والوسائط",
  "Cover Image": "صورة الغلاف",
  "Total Photos": "إجمالي الصور",
  "Basic Information": "المعلومات الأساسية",
  "Edit": "تعديل",
  "Min Area": "أدنى مساحة",
  "Max Area": "أقصى مساحة",
  "Please login to manage your properties.":
      "يرجى تسجيل الدخول لإدارة عقاراتك.",
  "Please login to access your chats.":
      "يرجى تسجيل الدخول للوصول إلى محادثاتك.",
  "Please enable location services.": "يرجى تفعيل خدمات الموقع.",
  "Permission": "الإذن",
  "Location permission denied.": "تم رفض إذن الموقع.",
  "Unable to detect current location.": "تعذر تحديد موقعك الحالي.",
  "Featured": "مميز",
  "Property ID is not available": "معرف العقار غير متوفر",
  "Boost Your Property": "عزّز عقارك",
  "Get more views and reach more buyers":
      "احصل على مشاهدات أكثر وتواصل مع مشترين أكثر",
  "Explore Now": "استكشف الآن",
  "What's your": "ما هي قيمة",
  "villa worth?": "فيلتك؟",
  "Find out in 30 seconds with our AI-powered valuation.":
      "اكتشف ذلك خلال 30 ثانية باستخدام تقييمنا المدعوم بالذكاء الاصطناعي.",
  "Get my free estimate": "احصل على تقييمي المجاني",
  "100% secure": "آمن بنسبة 100%",
  "AI-powered valuation": "تقييم مدعوم بالذكاء الاصطناعي",
  "Real market insights": "رؤى حقيقية عن السوق",
  "No nearby properties": "لا توجد عقارات قريبة",
  "Profile updated successfully": "تم تحديث الملف الشخصي بنجاح",
  "Version 1.0.0": "الإصدار 1.0.0",
  "Please login to edit your profile.": "يرجى تسجيل الدخول لتعديل ملفك الشخصي.",
  "Please login to view your saved properties.":
      "يرجى تسجيل الدخول لعرض عقاراتك المحفوظة.",
  "Please login to view your featured properties.":
      "يرجى تسجيل الدخول لعرض عقاراتك المميزة.",
  "Please login to contact support.": "يرجى تسجيل الدخول للتواصل مع الدعم.",
  "Guest": "ضيف",
  "No Email": "لا يوجد بريد إلكتروني",
  "USER": "مستخدم",
  "No review message available": "لا تتوفر رسالة مراجعة",
  "Issue: ": "المشكلة: ",
  "Edit and Resubmit": "تعديل وإعادة الإرسال",
  "Unable to report": "تعذر الإبلاغ",
  "Property information is not available.": "معلومات العقار غير متوفرة.",
  "Maximum Selection": "الحد الأقصى للاختيار",
  "You can compare only two properties.": "يمكنك مقارنة عقارين فقط.",
  "Select up to 2 properties": "اختر حتى عقارين",
  "Search Property": "البحث عن عقار",
  "Price Negotiable": "السعر قابل للتفاوض",
  "Contact Verified": "بيانات التواصل موثقة",
  "Nearby Places": "الأماكن القريبة",
  "Compare": "قارن",
  "Property Comparison": "مقارنة العقارات",
  "Change Selection": "تغيير الاختيار",
  "Invalid OTP": "رمز التحقق غير صحيح",
  "Please enter your WhatsApp number": "يرجى إدخال رقم الواتساب الخاص بك",
  "Please enter a valid phone number": "يرجى إدخال رقم هاتف صحيح",
  "Please enter the OTP": "يرجى إدخال رمز التحقق",
  "OTP must be 6 digits": "يجب أن يتكون رمز التحقق من 6 أرقام",
  "Failed to send OTP": "فشل إرسال رمز التحقق",
  "Search dealers": "البحث عن الوسطاء",
  "No dealers found": "لا يوجد وسطاء",
  "No matching dealers": "لا يوجد وسطاء مطابقون",
  "Website": "الموقع الإلكتروني",
  "Chat": "محادثة",
  "Active": "نشط",
  "Inactive": "غير نشط",
  "Trade No": "رقم السجل التجاري",
  "Active Since": "نشط منذ",
  "Contact Information": "معلومات التواصل",
  "Subscription Plan": "خطة الاشتراك",
  "Valid Till": "صالح حتى",
  "Statistics": "الإحصائيات",
  "Total Properties": "إجمالي العقارات",
  "Active Listings": "الإعلانات النشطة",
  "Sold Properties": "العقارات المباعة",
  "Listings": "إعلانات",
  "Valid for": "صالح لمدة",
  "Days": "أيام",
  "Boost Discount": "خصم على الترويج",
  "Priority Support": "دعم ذو أولوية",
  "About Company": "عن الشركة",
  "Show Less": "عرض أقل",
  "Read More": "قراءة المزيد",
  "No properties yet": "لا توجد عقارات بعد",
  "FOR SALE": "للبيع",
  "FOR RENT": "للإيجار",
  "Please wait...": "يرجى الانتظار...",
  "Sending...": "جارٍ الإرسال...",
  "Verifying...": "جارٍ التحقق...",
  "Google": "جوجل",
  "Apple": "آبل",
  "Discover premium villas and properties\nin the best locations across Qatar":
      "اكتشف أفضل الفلل والعقارات الفاخرة في أرقى المواقع في قطر",
  "Login Required": "تسجيل الدخول مطلوب",
  "Please sign in to use this feature.":
      "يرجى تسجيل الدخول لاستخدام هذه الميزة.",
  "Not Now": "ليس الآن",
  "Login": "تسجيل الدخول",
  "Bad Request": "طلب غير صالح",
  "Unauthorized": "غير مصرح به",
  "Forbidden": "ممنوع",
  "Not Found": "غير موجود",
  "Conflict": "تعارض في البيانات",
  "Invalid Data": "بيانات غير صالحة",
  "Server Error": "خطأ في الخادم",
  "No Internet Connection": "لا يوجد اتصال بالإنترنت",
  "Request timed out. Please check your internet connection and try again.":
      "انتهت مهلة الطلب. يرجى التحقق من اتصالك بالإنترنت والمحاولة مرة أخرى.",
  "Property unavailable": "العقار غير متاح",
  "Unable to open this property": "تعذر فتح هذا العقار",
  "Something went wrong": "حدث خطأ ما",
  "Upload Failed": "فشل الرفع",
  "Not Connected": "غير متصل",
  "Please wait for chat to connect.": "يرجى الانتظار حتى تتصل المحادثة.",
  "Location Disabled": "الموقع غير مفعّل",
  "Permission Denied": "تم رفض الإذن",
  "Location permission is required.": "إذن الوصول إلى الموقع مطلوب.",
  "Enable location permission from settings.":
      "قم بتفعيل إذن الموقع من الإعدادات.",
  "Location Error": "خطأ في تحديد الموقع",
  "Invalid support tickets response": "استجابة غير صالحة لطلبات الدعم",
  "Invalid create ticket response": "استجابة غير صالحة لإنشاء الطلب",
  "Ticket ID is missing": "معرّف الطلب مفقود",
  "Invalid support ticket details response":
      "استجابة غير صالحة لتفاصيل طلب الدعم",
  "Choose Location": "اختر الموقع",
  "Office Building": "مبنى مكاتب",
  "Commercial Building": "مبنى تجاري",
  "Use Current Location": "استخدام الموقع الحالي",
  "Detect location using GPS": "تحديد الموقع باستخدام نظام تحديد المواقع (GPS)",
  "Search city, area or landmark": "البحث عن مدينة أو منطقة أو معلم",
  "Search for a location": "البحث عن موقع",
  "No locations found": "لم يتم العثور على مواقع",
  "Search by city, area or landmark": "البحث حسب المدينة أو المنطقة أو المعلم",
  "Try another keyword": "جرّب كلمة رئيسية أخرى",
  "Browse by Category": "تصفح حسب الفئة",
  "dream villas": "فلل الأحلام",
  "Amenities & Features": "المرافق والميزات",
  "Insights": "رؤى",
  "Negotiable": "قابل للتفاوض",
  "View on Google Maps": "عرض على خرائط Google",
  "Compare Properties": "مقارنة العقارات",
  "Compare this property with similar listings":
      "قارن هذا العقار مع العقارات المشابهة",
  "Nearby": "بالقرب مني",
  "Sort": "ترتيب",
  "Newest": "الأحدث",
  "Oldest": "الأقدم",
  "Price: Low to High": "السعر: من الأقل إلى الأعلى",
  "Price: High to Low": "السعر: من الأعلى إلى الأقل",
  "Area: Low to High": "المساحة: من الأقل إلى الأعلى",
  "Area: High to Low": "المساحة: من الأعلى إلى الأقل",
  "VILLA": "فيلا",
  "APARTMENT": "شقة",
  "TOWNHOUSE": "منزل مستقل",
  "PENTHOUSE": "بنتهاوس",
  "STUDIO": "استوديو",
  "COMMERCIAL": "تجاري",
  "LAND": "أرض",
  "All Locations": "جميع المواقع",
  "Area (SQM)": "المساحة (م²)",
  "Read less": "قراءة أقل",
  "Read more": "قراءة المزيد",
  "Property Insights": "إحصاءات العقار",
  "Track how people are discovering and engaging with your property.":
      "تتبع كيفية اكتشاف الأشخاص لعقارك وتفاعلهم معه.",
  "Last": "آخر",
  "days": "أيام",
  "Totals": "الإجمالي",
  "All Time": "كل الوقت",
  "This Period Summary": "ملخص هذه الفترة",
  "Views": "المشاهدات",
  "Reach": "الوصول",
  "Impressions": "مرات الظهور",
  "WhatsApp Clicks": "نقرات واتساب",
  "Messages Started": "الرسائل التي بدأت",
  "Users Engaged": "المستخدمون المتفاعلون",
  "You received ": "لقد تلقيت ",
  "more": "أكثر",
  "fewer": "أقل",
  "views": "مشاهدة",
  "than the previous period.": "مقارنة بالفترة السابقة.",
  "Views Over Time": "المشاهدات بمرور الوقت",
  "Data is updated daily. All times are in your local timezone.":
      "يتم تحديث البيانات يوميًا. جميع الأوقات حسب منطقتك الزمنية المحلية.",
  "Insights will start appearing here once your property gets activity.":
      "ستبدأ الإحصاءات بالظهور هنا بمجرد أن يبدأ عقارك في تلقي نشاط.",
  "My Featured": "مميزاتي",
  "Password": "كلمة المرور",
  // "Account": "الحساب",
  "Update your account information": "تحديث معلومات حسابك",
  "Featured Plans": "خطط التمييز",
  "Manage your active and past featured property plans":
      "إدارة خطط العقارات المميزة الحالية والسابقة",
  // "Unable to load featured properties": "تعذر تحميل العقارات المميزة",
  // "Try Again": "حاول مرة أخرى",
  "No matching properties found": "لم يتم العثور على عقارات مطابقة",
  "No featured properties available": "لا توجد عقارات مميزة متاحة",
  "Try searching with another property name":
      "حاول البحث باستخدام اسم عقار آخر",
  "Your featured properties will appear here": "ستظهر عقاراتك المميزة هنا",
  "Clear Search": "مسح البحث",
  "Villas Qatar Payment Receipt": "إيصال دفع Villas Qatar",
  "Unable to generate receipt. Please try again":
      "تعذر إنشاء الإيصال. يرجى المحاولة مرة أخرى",
  "Select Language": "اختر اللغة",
  "Ask AI to find your villa...":
      "اطلب من الذكاء الاصطناعي العثور على فيلتك...",
  // "100% secure": "آمن بنسبة 100%",
  // "AI-powered valuation": "تقييم مدعوم بالذكاء الاصطناعي",
  // "Real market insights": "رؤى حقيقية عن السوق",
  "Your offer will be sent directly to the property owner. You can continue the conversation through chat":
      "سيتم إرسال عرضك مباشرة إلى مالك العقار. يمكنك متابعة المحادثة عبر الدردشة",
  "Share Property": "مشاركة العقار",
  "Share on WhatsApp": "المشاركة عبر واتساب",
  "Unable to share": "تعذرت المشاركة",
  "Could not open WhatsApp": "تعذر فتح واتساب",
  "Copy Link": "نسخ الرابط",
  "Property Description": "وصف العقار",
  "Muncipality": "البلدية",
  "Review Listing": "مراجعة الإعلان",
  "Verify all details before submitting": "تحقق من جميع التفاصيل قبل الإرسال",
  "Are you sure you want to mark this property as sold?":
      "هل أنت متأكد من رغبتك في وضع علامة تم البيع على هذا العقار؟",
  "This property will be updated as sold and will no longer be available as an active listing":
      "سيتم تحديث حالة هذا العقار إلى تم البيع ولن يكون متاحًا بعد الآن كإعلان نشط",
  "No visit requests": "لا توجد طلبات زيارة",
  "Visit requests for your properties will appear here":
      "ستظهر طلبات زيارة عقاراتك هنا",
  "No scheduled visits": "لا توجد زيارات مجدولة",
  "Properties you schedule for a visit will appear here":
      "ستظهر هنا العقارات التي تقوم بجدولة زيارتها",
  "Skip": "تخطي",
  "Premium living Prime locations\nThe lifestyle you deserve":
      "معيشة فاخرة، مواقع مميزة\nالحياة التي تستحقها",
  "or continue with": "أو تابع باستخدام",
  "By continuing, you agree to our\n": "بمتابعة الاستخدام، فإنك توافق على\n",
  "Terms of Use": "شروط الاستخدام",
  "Privacy Policy": "سياسة الخصوصية",
  'No Properties Listed': 'لا توجد عقارات مدرجة',
  'This dealer hasn\'t added any listings yet':
      'لم يقم هذا الوسيط بإضافة أي عقارات بعد',
  "Seller Properties": "عقارات البائع",

  "This seller has no more properties to show":
      "لا يملك هذا البائع المزيد من العقارات لعرضها",
  'View all': 'عرض الكل',
  "This period": "هذه الفترة",
  "Performance Trend": "اتجاه الأداء",
  "Top Listings": "أفضل العقارات",
  "No listings in this period": "لا توجد عقارات في هذه الفترة",

  "Villas Qatar": "فلل قطر",
  "DEALER PORTAL": "بوابة الوسيط العقاري",
  "Dealer": "الوسيط العقاري",
  "Log out": "تسجيل الخروج",

  "Analytics": "التحليلات",
  "Performance overview for": "نظرة عامة على أداء",
  "your agency": "وكالتك العقارية",

  // Subscription
  "No active plan": "لا توجد خطة نشطة",
  "Active subscription": "الاشتراك نشط",

  "DAYS LEFT": "يوم متبقي",
  "Max listings": "الحد الأقصى للعقارات",
  "Featured left": "العقارات المميزة المتبقية",
  "Ends": "ينتهي",

  // Statistics
  "Total Listings": "إجمالي العقارات",
  "Total Views": "إجمالي المشاهدات",

  "Conversations": "المحادثات",
  "Units Sold": "الوحدات المباعة",
  "Site Visits": "الزيارات للموقع",

  "Sold": "مباع",

  "in this period": "في هذه الفترة",
  "reach": "الوصول",
  "this period": "هذه الفترة",
  "unique users chatted": "مستخدمون فريدون بدأوا محادثات",
  "No sales this period": "لا توجد مبيعات في هذه الفترة",
  "pending": "قيد الانتظار",
  "accepted": "مقبول",

  // Chart
  "No trend data for this period": "لا توجد بيانات للاتجاه في هذه الفترة",

  "Visits": "الزيارات",

  // Listing card
  "QAR": "ر.ق",

  // Filters
  "Filter Analytics": "تصفية التحليلات",
  "Date range": "نطاق التاريخ",
  "Granularity": "الفترة الزمنية",

  "Listing": "العقار",
  "Staff member": "عضو فريق العمل",
  "Property type": "نوع العقار",
  "Area name": "اسم المنطقة",
  "e.g. Pearl": "مثال: اللؤلؤة",

  "All listings": "جميع العقارات",
  "All staff": "جميع الموظفين",
  "All types": "جميع الأنواع",

  // Granularity
  "Daily": "يومي",
  "Weekly": "أسبوعي",
  "Monthly": "شهري",
  "Yearly": "سنوي",

  // Purpose
  "SALE": "بيع",
  "RENT": "إيجار",

  "REJECTED": "مرفوض",
  "RESUBMITTED": "تمت إعادة التقديم",
  "SOLD": "مباع",
  "PENDING_PAYMENT": "بانتظار الدفع",

  "Penthouse": "بنتهاوس",

  "Login as ": "تسجيل الدخول كـ ",

  "Enter your dealer account email and\npassword to continue":
      "أدخل البريد الإلكتروني لحساب الوسيط وكلمة المرور للمتابعة",
  "Email or phone": "البريد الإلكتروني أو رقم الهاتف",

  "Logging in...": "جارٍ تسجيل الدخول...",

  "The dealer portal is for agencies managing listings, leads and analytics for their team.":
      "بوابة الوسطاء مخصصة للوكالات لإدارة العقارات والعملاء المحتملين والتحليلات الخاصة بفريقهم.",

  "Invoice Details": "تفاصيل الفاتورة",
  "PDF is not available for this invoice": "ملف PDF غير متوفر لهذه الفاتورة",
  "Please login to download the invoice.": "يرجى تسجيل الدخول لتنزيل الفاتورة.",
  "Unable to download invoice PDF": "تعذر تنزيل ملف PDF الخاص بالفاتورة",

  "Invoice Number": "رقم الفاتورة",
  "Total Amount": "المبلغ الإجمالي",
  "Billed To": "فاتورة إلى",
  "Payment Information": "معلومات الدفع",

  "Company": "الشركة",
  "Trade Number": "الرقم التجاري",
  "Address": "العنوان",

  "Payment Method": "طريقة الدفع",
  "Paid At": "تاريخ الدفع",

  "Emailed At": "تاريخ إرسال البريد الإلكتروني",

  "Line Items": "بنود الفاتورة",

  "Item": "البند",
  "Unit Price": "سعر الوحدة",
  "Qty": "الكمية",
  "Amount": "المبلغ",

  "Subtotal": "المجموع الفرعي",
  "VAT (@rate%)": "ضريبة القيمة المضافة (@rate%)",

  "PDF": "ملف PDF",
  "Created At": "تاريخ الإنشاء",
  "Updated At": "تاريخ التحديث",

  "Copied to clipboard": "تم النسخ إلى الحافظة",

  "Paid": "مدفوعة",
  "Unknown": "غير معروف",
  "Login as Dealer": "تسجيل الدخول كتاجر",

  "My Invoices": "فواتيري",
  "Search invoices": "البحث في الفواتير",

  "No invoices found": "لم يتم العثور على فواتير",
  "Your invoices will appear here once available":
      "ستظهر فواتيرك هنا عند توفرها",

  "Invoice": "فاتورة",
  "Invoices": "فواتير",

  "Enter your email": "أدخل بريدك الإلكتروني",
  "Select Municipality": "اختر البلدية",
  "Use GPS": "استخدام نظام تحديد المواقع",
  "This image will be shown first in listings":
      "ستظهر هذه الصورة أولاً في القوائم",
  "Submit Property": "إرسال العقار",

  "Download Receipt": "تنزيل الإيصال",

  // --- Auto-added: full localization sweep 2026-08-19 ---
  " (150 Reviews)": " (150 تقييم)",
  " and ": " و ",
  "Accept": "قبول",
  "Add your details to personalize\nyour experience":
      "أضف بياناتك لتخصيص\nتجربتك",
  "Additional details": "تفاصيل إضافية",
  "Amount Paid": "المبلغ المدفوع",
  "Apple sign-in failed": "فشل تسجيل الدخول عبر آبل",
  "Chat with Seller": "تواصل مع البائع",
  "Check": "تحقق",
  "Close": "إغلاق",
  "Complete Payment": "إتمام الدفع",
  "Currency": "العملة",
  "Dashboard": "لوحة التحكم",
  "Date not available": "التاريخ غير متوفر",
  "Describe your property": "صف عقارك",
  "Doha": "الدوحة",
  "Edit & Resubmit": "تعديل وإعادة الإرسال",
  "End Date": "تاريخ الانتهاء",
  "Ends today": "ينتهي اليوم",
  "Enter amount": "أدخل المبلغ",
  "Expired": "منتهي الصلاحية",
  "Failed": "فشل",
  "Failed to complete profile. Please try again.":
      "تعذر إكمال الملف الشخصي. يرجى المحاولة مرة أخرى.",
  "Failed to update profile": "تعذر تحديث الملف الشخصي",
  "Find your dream villa": "اعثر على فيلا أحلامك",
  "Free Estimates Used": "التقديرات المجانية المستخدمة",
  "Google sign-in failed": "فشل تسجيل الدخول عبر جوجل",
  "ID #": "رقم التعريف #",
  "Inappropriate content": "محتوى غير لائق",
  "Incorrect listing information": "معلومات الإعلان غير صحيحة",
  "Invalid OTP. Please try again.":
      "رمز التحقق غير صحيح. يرجى المحاولة مرة أخرى.",
  "Invalid email/phone or password.":
      "البريد الإلكتروني/رقم الهاتف أو كلمة المرور غير صحيحة.",
  "Invalid property response": "استجابة غير صالحة للعقار",
  "Issue": "المشكلة",
  "Let's complete\n": "لنكمل\n",
  "your profile": "ملفك الشخصي",
  "LISTED": "مُدرج",
  "Listing Rejected": "تم رفض الإعلان",
  "Location not available": "الموقع غير متوفر",
  "Logged-in user information is not available.":
      "معلومات المستخدم المسجل دخوله غير متوفرة.",
  "Login is required to make an offer.": "يجب تسجيل الدخول لتقديم عرض.",
  "Login is required to save properties to your wishlist.":
      "يجب تسجيل الدخول لحفظ العقارات في قائمة المفضلة.",
  "Message": "رسالة",
  "Misleading or fake listing": "إعلان مضلل أو مزيف",
  "No properties found": "لا توجد عقارات",
  "Offer Submitted!": "تم إرسال العرض!",
  "Owner": "المالك",
  "Pay": "الدفع",
  "Please add a cover image": "يرجى إضافة صورة غلاف",
  "Please add at least one property photo":
      "يرجى إضافة صورة واحدة على الأقل للعقار",
  "Please enter a property description": "يرجى إدخال وصف للعقار",
  "Please enter a valid email address": "يرجى إدخال بريد إلكتروني صحيح",
  "Please enter a valid offer amount.": "يرجى إدخال قيمة عرض صحيحة.",
  "Please enter the address": "يرجى إدخال العنوان",
  "Please enter the area": "يرجى إدخال المساحة",
  "Please enter the number of bathrooms": "يرجى إدخال عدد الحمامات",
  "Please enter the number of bedrooms": "يرجى إدخال عدد غرف النوم",
  "Please enter the price": "يرجى إدخال السعر",
  "Please enter the property name": "يرجى إدخال اسم العقار",
  "Please enter your contact number": "يرجى إدخال رقم التواصل الخاص بك",
  "Please enter your email address": "يرجى إدخال بريدك الإلكتروني",
  "Please enter your email or phone":
      "يرجى إدخال بريدك الإلكتروني أو رقم هاتفك",
  "Please enter your full name": "يرجى إدخال اسمك الكامل",
  "Please enter your offer amount.": "يرجى إدخال قيمة عرضك.",
  "Please enter your password": "يرجى إدخال كلمة المرور",
  "Please login to continue": "يرجى تسجيل الدخول للمتابعة",
  "Please login to report a listing.": "يرجى تسجيل الدخول للإبلاغ عن إعلان.",
  "Please select Sale or Rent": "يرجى اختيار بيع أو إيجار",
  "Please select a municipality": "يرجى اختيار البلدية",
  "Please select a property type": "يرجى اختيار نوع العقار",
  "Please select the property location on the map":
      "يرجى تحديد موقع العقار على الخريطة",
  "Premium Living. Perfect Location": "معيشة راقية. موقع مثالي",
  "Property ID is missing..": "معرّف العقار غير موجود..",
  "Property is no longer available": "العقار لم يعد متاحًا",
  "Property link copied to clipboard": "تم نسخ رابط العقار إلى الحافظة",
  "Property link is not available": "رابط العقار غير متوفر",
  "Property marked as sold successfully": "تم تحديد العقار كمباع بنجاح",
  "Property not found": "لم يتم العثور على العقار",
  "Property owner": "مالك العقار",
  "Provide additional information that may help us review this listing.":
      "قدم معلومات إضافية قد تساعدنا في مراجعة هذا الإعلان.",
  "Qatar": "قطر",
  "Reject": "رفض",
  "Report listing": "الإبلاغ عن الإعلان",
  "Reports are reviewed by our support team.":
      "تتم مراجعة البلاغات من قبل فريق الدعم لدينا.",
  "Resubmitted": "تمت إعادة التقديم",
  "Scheduled": "مجدولة",
  "Send": "إرسال",
  "Something went wrong. Please try again.":
      "حدث خطأ ما. يرجى المحاولة مرة أخرى.",
  "Start Date": "تاريخ البدء",
  "Submit Offer": "إرسال العرض",
  "Suspected scam or fraud": "اشتباه في احتيال أو نصب",
  "Tell us what's wrong with this property.":
      "أخبرنا بما هو خاطئ في هذا العقار.",
  "Thank you. Our support team will review this listing.":
      "شكرًا لك. سيقوم فريق الدعم لدينا بمراجعة هذا الإعلان.",
  "Total Paid": "إجمالي المدفوع",
  "Unable to mark property as sold": "تعذر تحديد العقار كمباع",
  "Unable to open the page": "تعذر فتح الصفحة",
  "Unable to start checkout. Please try again.":
      "تعذر بدء عملية الدفع. يرجى المحاولة مرة أخرى.",
  "Unable to start payment. Please try again.":
      "تعذر بدء عملية الدفع. يرجى المحاولة مرة أخرى.",
  "Update Property": "تحديث العقار",
  "Verification Required": "التحقق مطلوب",
  "Verified Contact": "بيانات تواصل موثقة",
  "Verified Seller": "بائع موثّق",
  "Verify Number": "تحقق من الرقم",
  "Verify OTP": "تحقق من الرمز",
  "Visit Request": "طلب زيارة",
  "Visitor": "زائر",
  "Why are you reporting this listing?": "لماذا تقوم بالإبلاغ عن هذا الإعلان؟",
  "Would you like to accept or reject this visit request?":
      "هل ترغب في قبول أو رفض طلب الزيارة هذا؟",
  "You can try again anytime.": "يمكنك المحاولة مرة أخرى في أي وقت.",
  "Your Offer Details": "تفاصيل عرضك",
  "Choose the reason that best describes the issue.":
      "اختر السبب الذي يصف المشكلة بشكل أفضل.",
  "العربية": "العربية",
  "@count Invoice": "@count فاتورة",
  "@count Invoices": "@count فواتير",
  // --- Auto-added: follow-up sweep 2026-08-19 ---
  "We'll never share your information with anyone":
      "لن نشارك معلوماتك مع أي جهة أبدًا",
  "Login is required to schedule a property visit.":
      "يجب تسجيل الدخول لجدولة زيارة للعقار.",
  "Please login to save properties to your wishlist.":
      "يرجى تسجيل الدخول لحفظ العقارات في قائمة المفضلة.",
  "Please review the issue below and update your listing before submitting it again.":
      "يرجى مراجعة المشكلة أدناه وتحديث إعلانك قبل إعادة تقديمه.",
  "Sign in to keep estimating.": "سجّل الدخول لمتابعة التقدير.",
  "Your offer has been sent successfully.\nThe seller will review it and contact you soon.":
      "تم إرسال عرضك بنجاح.\nسيقوم البائع بمراجعته والتواصل معك قريبًا.",
  "Payment Pending": "بانتظار الدفع",
  "ready_to_buy_property": "أنا مستعد لشراء هذا العقار مقابل @currency @amount",

  // ─── Home page (web-aligned redesign) ─────────────────────────────
  "LIVE A BETTER TOMORROW": "عش غداً أفضل",
  "Find your perfect": "اعثر على منزلك",
  "home in": "المثالي في",
  "Discover homes and properties across Qatar — buy, sell or rent with confidence.": "اكتشف المنازل والعقارات في جميع أنحاء قطر — للشراء أو البيع أو الإيجار بسهولة وثقة.",
  "Search by location, area or landmark": "ابحث بالموقع أو المنطقة أو المعلم",
  "Search by Property Type": "ابحث حسب نوع العقار",
  "Find the right property type for your search": "اعثر على نوع العقار المناسب لبحثك",
  "Handpicked premium listings for you": "قوائم فاخرة مختارة بعناية لك",
  "Listings closest to where you are right now": "أقرب العقارات إلى موقعك الحالي",
  "Trusted agencies listing across Qatar": "وكالات موثوقة تعرض عقاراتها في جميع أنحاء قطر",
  "Popular Places in Qatar": "الأماكن الشائعة في قطر",
  "Discover properties in the most sought-after locations": "اكتشف العقارات في أكثر المواقع طلباً",
  "Explore area": "استكشف المنطقة",
  "From": "من",
  "Smart & Fast": "ذكي وسريع",
  "Estimate your property price with AI": "قدّر سعر عقارك بالذكاء الاصطناعي",
  "Get instant property valuation based on market trends and real data in Qatar.": "احصل على تقييم فوري لعقارك بناءً على اتجاهات السوق والبيانات الحقيقية في قطر.",
  "Estimated Value": "القيمة التقديرية",
  "from last month": "منذ الشهر الماضي",
  "Lusail City": "مدينة لوسيل",
  "Estimate Now": "قدّر الآن",
  "Takes less than 2 minutes": "يستغرق أقل من دقيقتين",
  "AI Powered": "مدعوم بالذكاء الاصطناعي",
  "Accurate & Reliable": "دقيق وموثوق",
  "FOR REAL ESTATE DEALERS": "للوكلاء العقاريين",
  "Grow Your Business": "نمِّ أعمالك",
  "with": "مع",
  "List your properties, manage inquiries and reach thousands of buyers and renters — all in one place.": "أدرج عقاراتك، وأدِر الاستفسارات، وصل إلى آلاف المشترين والمستأجرين — كل ذلك في مكان واحد.",
  "Easy Property Management": "إدارة سهلة للعقارات",
  "Add, edit and manage your listings with ease.": "أضف وعدّل وأدِر قوائمك بكل سهولة.",
  "Connect with Genuine Clients": "تواصل مع عملاء حقيقيين",
  "Receive and manage inquiries in real-time.": "استقبل وأدِر الاستفسارات في الوقت الفعلي.",
  "Track Your Performance": "تتبع أدائك",
  "Get insights on views, leads and conversions.": "احصل على رؤى حول المشاهدات والعملاء المحتملين والتحويلات.",
  "Grow with Flexible Plans": "نمو بخطط مرنة",
  "Choose a plan that fits your business and scale faster.": "اختر خطة تناسب أعمالك وتوسع بشكل أسرع.",
  "Become a Dealer": "أصبح وكيلاً",

  // Property search
  "Find your perfect home in Qatar": "اعثر على منزل أحلامك في قطر",
  "Premium properties across Qatar": "عقارات مميزة في جميع أنحاء قطر",
  "Search by location, area or property": "ابحث بالموقع أو المنطقة أو العقار",
  "Searching...": "جارٍ البحث...",
  "@count Properties Found": "تم العثور على @count عقار",
  "Clear Filters": "مسح الفلاتر",
  "Try adjusting your search or filters": "حاول تعديل البحث أو الفلاتر",
  "All Types": "جميع الأنواع",
  "Bed": "غرفة نوم",
  "Bath": "حمام",

  // Property details redesign
  "Asking Price": "السعر المطلوب",
  "per year": "سنوياً",
  "Listed By": "معروض بواسطة",
  "View all listings": "عرض جميع العقارات",
  "Reference": "الرقم المرجعي",
  "Country": "الدولة",

  // Home social media section
  "Join us on social media": "انضم إلينا على وسائل التواصل الاجتماعي",
  "Follow us for new listings, market updates and exclusive offers":
      "تابعنا للاطلاع على أحدث العقارات وتحديثات السوق والعروض الحصرية",
  "Could not open the link": "تعذر فتح الرابط",

  // List / edit property wizard redesign
  "Add high quality photos to attract more buyers": "أضف صوراً عالية الجودة لجذب المزيد من المشترين",
  "Back": "رجوع",
  "Buyers will use these details to reach you": "سيستخدم المشترون هذه البيانات للتواصل معك",
  "Change": "تغيير",
  "Confirm your number is verified on WhatsApp.": "تأكد من أن رقمك موثّق على واتساب.",
  "Contact Details": "بيانات التواصل",
  "Coordinates": "الإحداثيات",
  "Couldn't load options": "تعذّر تحميل الخيارات",
  "Cover Photo": "صورة الغلاف",
  "Discard": "تجاهل",
  "Discard listing?": "تجاهل الإعلان؟",
  "Edit Property": "تعديل العقار",
  "Highlight what makes your property special": "أبرز ما يميّز عقارك",
  "Keep Editing": "متابعة التعديل",
  "No property types available": "لا توجد أنواع عقارات متاحة",
  "No results found": "لا توجد نتائج",
  "Open to offers from buyers": "مفتوح لعروض المشترين",
  "Pin Location": "تحديد الموقع",
  "Price is negotiable": "السعر قابل للتفاوض",
  "Pricing": "التسعير",
  "Property Information": "معلومات العقار",
  "Remove": "إزالة",
  "Selected": "محدد",
  "Show all": "عرض الكل",
  "Show less": "عرض أقل",
  "Specifications": "المواصفات",
  "Step @current of @total": "الخطوة @current من @total",
  "Use clear, well-lit photos of every room": "استخدم صوراً واضحة ومضيئة لكل غرفة",
  "Use my location": "استخدم موقعي",
  "Use your current position or search for the place to fill in the address automatically.": "استخدم موقعك الحالي أو ابحث عن المكان لتعبئة العنوان تلقائياً.",
  "You can add up to 20 photos": "يمكنك إضافة حتى 20 صورة",
  "Your progress will be lost.": "سيتم فقدان ما أدخلته.",
};
