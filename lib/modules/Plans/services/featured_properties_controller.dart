...
class FeaturedPropertiesController extends GetxController {
  ...
  List<FeaturedProperty> getProperties(FeaturedLocation location) {
    return _properties[location.apiValue] ?? [];
  }

  bool isLoading(FeaturedLocation location) {
    return _loading[location.apiValue] ?? false;
  }

  bool isLoadingMore(FeaturedLocation location) {
    return _loadingMore[location.apiValue] ?? false;
  }

  bool hasMore(FeaturedLocation location) {
    return _hasMore[location.apiValue] ?? true;
  }

  String getError(FeaturedLocation location) {
    return _errors[location.apiValue] ?? '';
  }

  int getTotal(FeaturedLocation location) {
  ...
  List<FeaturedProperty> get homeProperties =>
      getProperties(FeaturedLocation.homePage);
  ...
  List<FeaturedProperty> get propertyDetailProperties =>
      getProperties(FeaturedLocation.propertyDetailPage);
  ...
  Future<void> fetchFeaturedProperties({
    ...
    if (loadMore) {
      ...
      if (!forceRefresh && (_properties[key]?.isNotEmpty ?? false)) {
        return;
      }
      ...
      if (forceRefresh) {
        debugPrint('🔄 FORCE REFRESH: Clearing old $key data');
    ...
    try {
      ...
      final int currentPage = _pages[key] ?? 1;
      ...
      if (response is! Map<String, dynamic>) {
        throw Exception('Invalid featured properties response');
      }
      ...
      final model = FeaturedPropertiesResponse.fromJson(response);

      final List<FeaturedProperty> newItems = model.data;
      for (int i = 0; i < newItems.length; i++) {
        final FeaturedProperty featured = newItems[i];
      ...
      if (loadMore) {
        final List<FeaturedProperty> existing = _properties[key] ?? [];

        final Set<String> existingIds = existing
            .map((e) => e.listing.id)
            .toSet();

        final List<FeaturedProperty> uniqueNewItems = newItems.where((item) {
          return !existingIds.contains(item.listing.id);
        }).toList();
        _properties[key] = [...existing, ...uniqueNewItems];
      } else {
      ...
      final int loadedCount = _properties[key]?.length ?? 0;

      // ==========================================================
      // HAS MORE
      // ==========================================================

      _hasMore[key] = loadedCount < model.total && newItems.isNotEmpty;
      ...
      _errors[key] = e.toString().replaceFirst('Exception: ', '');
    } finally {
      if (loadMore) {
